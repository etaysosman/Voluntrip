document.addEventListener('DOMContentLoaded', function() {
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', function(event) {
            ...
            if (country.value.trim().length < 2 || !/^[a-zA-Z\s]+$/.test(country.value.trim())) {
                alert("Country Error: Must be at least 2 characters, letters only (no numbers or special characters).");
                country.focus();
                return;
            }
    ...
    if (createActivityForm) {
        createActivityForm.addEventListener('submit', function(event) {
            ...
            const location = document.getElementById('actLocation');
            const orgname = document.getElementById('actOrganizer');
            ...
            if (location.value.trim().length < 2 || !lettersRegex.test(location.value.trim())) {
                alert("Location Error: Please enter a valid location (must contain letters and be at least 2 characters).");
                location.focus();
                return;
            }

            if (orgname.value.trim().length < 2 || !lettersRegex.test(orgname.value.trim())) {
                alert("Organizer Error: Please enter a valid organizer name (must contain letters).");
                orgname.focus();
                return;
            }
    ...
    if (connectForm) {
        connectForm.addEventListener('submit', function(event) {
            ...
            const israeliPhonePattern = /^0(?:5[234890]\d{7})$/;

            if (!israeliPhonePattern.test(phoneVal)) {
                alert("Contact Phone Error: Must be a valid phone number starting with 05..");
                contactPhone.focus();
                return;
            }
            if (socialLink.value.trim() !== "" && !isValidUrl(socialLink.value.trim())) {
