...
document.addEventListener('DOMContentLoaded', function() {
    ...
    //E. NAV ACTIVE HIGHLIGHT 
    try {
        const current = (location.pathname.replace(/\/$/, '') || '/');
        document.querySelectorAll('#mainMenu .vt-nav__center a.vt-nav__link').forEach(link => {
            try {
                const href = (new URL(link.getAttribute('href'), location.origin).pathname.replace(/\/$/, '') || '/');
                if (href === current) {
                    link.classList.add('vt-active');
                    link.setAttribute('aria-current', 'page');
                }
            } catch (e) {
                // ignore invalid/malformed hrefs (external links etc.)
            }
        });
    } catch (e) {
        // if DOM isn't available or selectors change, fail silently
    }
